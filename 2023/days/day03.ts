...
function getBounds(x: number, y: number, size: number, grid: Grid): Bounds {
  return {
    xMin: Math.max(0, x - 1),
    xMax: Math.min(grid.length - 1, x + 1),
    yMin: Math.max(0, y - 1),
    yMax: Math.min(grid[x].length - 1, y + size + 1),
  };
}

function isNearSymbol(bounds: Bounds, grid: Grid): boolean {
  for (let i = bounds.xMin; i <= bounds.xMax; i++) {
    if (
      grid[i].join("").substring(bounds.yMin, bounds.yMax).match(SYMBOL_REGEX)
    ) {
      return true;
    }
  }
...
function calculateGearRatio(bounds: Bounds, grid: Grid): number {
  const adjacentPartNumbers: number[] = [];

  for (let i = bounds.xMin; i <= bounds.xMax; i++) {
    const rowString = grid[i].join("");
    Array.from(rowString.matchAll(/\d+/g))
      .forEach((match) => {
        if (match.index > bounds.yMax) return;
        if (match.index + match[0].length - 1 < bounds.yMin) return;
...
function findSumOfValidPartNumbers(input: string): number {
  let result = 0;
  ...
  grid.forEach((row, x) => {
    ...
    Array.from(rowString.matchAll(/(\d+)/g))
      .forEach((match) => {
        if (
          isNearSymbol(getBounds(x, match.index, match[0].length, grid), grid)
        ) {
...
function findSumOfGearRatios(input: string): number {
  let result = 0;
  ...
  grid.forEach((row, x) => {
    const rowString = row.join("");
    Array.from(rowString.matchAll(/(\*)/g))
      .forEach((match) => {
        result += calculateGearRatio(getBounds(x, match.index, 0, grid), grid);
      });
  });
...
export function day03() {
  const partOneOutput = findSumOfValidPartNumbers(PUZZLE_INPUT);

  console.log(`Part One: ${partOneOutput}`);

  const partTwoOutput = findSumOfGearRatios(PUZZLE_INPUT);

  console.log(`Part Two: ${partTwoOutput}`);
}

Deno.test(function day03Test() {
  assertEquals(findSumOfValidPartNumbers(EXAMPLE_INPUT), 4361);
  assertEquals(findSumOfValidPartNumbers(PUZZLE_INPUT), 550934);
  assertEquals(findSumOfGearRatios(EXAMPLE_INPUT), 467835);
  assertEquals(findSumOfGearRatios(PUZZLE_INPUT), 81997870);
});

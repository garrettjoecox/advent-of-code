...
const REGEXP = /one|two|three|four|five|six|seven|eight|nine/g;

function decodeCalibration(
  input: string,
  replaceNumberStrings = false,
): number {
  let sum = 0;
  input.split("\n")
    .forEach((line) => {
      ...
      if (replaceNumberStrings) {
        let matches = Array.from(line.matchAll(REGEXP));
        while (matches.length) {
          // this is kind of stupid, but when there are overlapping numbers, _both_ count. So eightwo is 82, twone is 21, etc
          // So what we're opting to do is from left to right only replace the first character of the number, that way the
          // rest of the block from part one works. Using some sort of iteration would probably be better here.
          const copy = line.split("");
          copy[matches[0].index] = NUMBER_MAP[matches[0][0]];
          line = copy.join("");

          matches = Array.from(line.matchAll(REGEXP));
        }
      }

      const numbers = Array.from(line.matchAll(/\d/g));
      const resultString = `${numbers[0]}${numbers.pop()}`;
...
export function day01() {
  const partOneOutput = decodeCalibration(PUZZLE_INPUT, false);

  console.log(`Part One: ${partOneOutput}`);

  const partTwoOutput = decodeCalibration(PUZZLE_INPUT, true);

  console.log(`Part Two: ${partTwoOutput}`);
}

Deno.test(function day01Test() {
  assertEquals(decodeCalibration(EXAMPLE_INPUT_PART_ONE, false), 142);
  assertEquals(decodeCalibration(PUZZLE_INPUT, false), 53080);
  assertEquals(decodeCalibration(EXAMPLE_INPUT_PART_TWO, true), 281);
  assertEquals(decodeCalibration(EXAMPLE_INPUT_EDGE_CASE, true), 363);
  assertEquals(decodeCalibration(PUZZLE_INPUT, true), 53268);
});
